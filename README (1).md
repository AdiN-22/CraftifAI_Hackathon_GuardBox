...
- **`Firmware.zip`** — full firmware source, organized by FirmGen into:
  - `app/` — application-level logic (alert state machine, arm/disarm handling)
  - `modules/` — individual driver modules (LED/output, switch input, IMU driver, ultrasonic driver, Wi-Fi/MQTT reporting)
  - `configs/` — centralized configuration (pins, thresholds, timing values)
  - `interfaces/`, `platforms/`, `services/`, `utils/` — supporting FirmGen-generated scaffolding
  - `docs/` — FirmGen-generated project documentation
  - `tests/`, `scripts/` — build/test support files
- **`CraftitifiAI_GuardBox_Project_export.zip`** — release-ready export from FirmGen, containing:
  - `firmware/` — packaged firmware build
  - `flash.bat` / `flash.sh` — one-step flashing scripts for Windows/Linux
  - `flash_config.json` — flash configuration
  - `README.md` — FirmGen-generated build/flash notes (separate from this top-level README)
